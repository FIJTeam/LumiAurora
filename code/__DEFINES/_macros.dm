...
#define show_browser(target, content, title) to_target(target, browse(findtext_char(content, "<html><head>")?replacetext_char(content, "<html><head>", "<html><head><meta charset='utf-8'>"):findtext_char(content, "<head>")?replacetext_char(content, "<head>", "<head><meta charset='utd-8'>"):"<head><meta charset='utf-8'>[content]", title)) // Thx to Proxima
