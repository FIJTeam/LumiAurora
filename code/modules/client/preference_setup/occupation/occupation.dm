...
/datum/category_item/player_setup_item/occupation/proc/show_faction_menu(mob/user, selected_faction) //note : selected faction is what you choose to see, pref.faction is the actual chosen faction
	...
	dat += "Подробнее об этой фракции вы можете узнать на <a href='byond://?src=[REF(user.client)];JSlink=wiki;wiki_page=[replacetext(faction.name, " ", "_")]'>вики</a>."

	if (selected_faction == pref.faction)
		dat += "<br>\[Фракция выбрана\]"
	else if (faction.can_select(pref,user))
		dat += "<br>\[<a href='byond://?src=[REF(src)];faction_select=[html_encode(selected_faction)]'>Выбрать фракцию</a>\]"
	else
