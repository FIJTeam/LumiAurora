/datum/admins/Topic(href, href_list)
	...
	if(href_list["dbsearchckey"] || href_list["dbsearchadmin"])
		...
		switch(href_list["simplemake"])
			...
			if("human")
				spawn_humanoid_species_admin(usr, M, delmob)
			if("slime")
				M.change_mob_type( /mob/living/carbon/slime , null, null, delmob )
			if("ai")
				M.change_mob_type( /mob/living/silicon/ai , null, null, delmob )
			if("robot")
			...
			if("constructarmored")
				M.change_mob_type( /mob/living/simple_animal/construct/armored , null, null, delmob )
			if("constructbuilder")
	...
	// Are now moved to banjob.dm. For use with the newer system.
		...
		if (istype(fax, /obj/item/paper))
			...
			for (var/page = 1, page <= B.pages.len, page++)
				...
				data += "<meta charset=\"UTF-8\"><A href='byond://?src=[REF(src)];AdminFaxViewPage=[page];paper_bundle=[REF(B)]'>Page [page] - [pageobj.name]</A><BR>"
